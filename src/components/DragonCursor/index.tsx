...
export default function DragonCursor() {
  ...
  const elemsRef = useRef<Array<{ use: SVGUseElement | null; x: number; y: number }>>([]);
  const radRef = useRef(0);
  const frmRef = useRef(Math.random());
  const radmRef = useRef(0);
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      radmRef.current = Math.min(widthRef.current / 2, heightRef.current / 2) - 20;
    };
    resize();
    ...
    pointerRef.current = { x: widthRef.current / 2, y: heightRef.current / 2 };

    // Create SVG definitions
    const svg = screenRef.current.ownerSVGElement;
    if (!svg) return;

    const defs = svg.querySelector('defs') || document.createElementNS(xmlns, 'defs');
    if (!svg.querySelector('defs')) {
      svg.insertBefore(defs, svg.firstChild);
    }

    // Head (Cabeza)
    const cabeza = document.createElementNS(xmlns, 'g');
    ...
    `;
    if (!defs.querySelector('#Cabeza')) {
      defs.appendChild(cabeza);
    }

    // Wings (Aletas)
    const aletas = document.createElementNS(xmlns, 'g');
    aletas.setAttribute('id', 'Aletas');
    aletas.innerHTML = `
      <linearGradient id="LinearGradID_1" gradientUnits="userSpaceOnUse" gradientTransform="matrix(0.0935974, 0, 0, 0.188782, -20.55, 0)" spreadMethod="pad" x1="-819.2" y1="0" x2="819.2" y2="0">
        ...
        <stop offset="1" style="stop-color:#007a99;stop-opacity:0.4" />
      </linearGradient>
    ...
    `;
    if (!defs.querySelector('#Aletas')) {
      defs.appendChild(aletas);
    }

    // Body segments (Espina)
    const espina = document.createElementNS(xmlns, 'g');
    espina.setAttribute('id', 'Espina');
    espina.innerHTML = `
      <linearGradient id="LinearGradID_2" gradientUnits="userSpaceOnUse" gradientTransform="matrix(0.0229492, 0, 0, -0.0152893, 0, 0.05)" spreadMethod="pad" x1="-819.2" y1="0" x2="819.2" y2="0">
        ...
        <stop offset="1" style="stop-color:#007a99;stop-opacity:1" />
      </linearGradient>
    ...
    `;
    if (!defs.querySelector('#Espina')) {
      defs.appendChild(espina);
    }
    ...
    const prepend = (use: string, i: number) => {
      ...
      if (screenRef.current) {
        screenRef.current.prepend(elem);
      }
    };
    ...
    const run = () => {
      ...
      let e = elemsRef.current[0];
      const ax = (Math.cos(3 * frmRef.current) * radRef.current * widthRef.current) / heightRef.current;
      const ay = (Math.sin(4 * frmRef.current) * radRef.current * heightRef.current) / widthRef.current;
      e.x += (ax + pointerRef.current.x - e.x) / 10;
      e.y += (ay + pointerRef.current.y - e.y) / 10;
      
      for (let i = 1; i < N; i++) {
        let e = elemsRef.current[i];
        let ep = elemsRef.current[i - 1];
        const a = Math.atan2(e.y - ep.y, e.x - ep.x);
        e.x += (ep.x - e.x + (Math.cos(a) * (100 - i)) / 5) / 4;
        e.y += (ep.y - e.y + (Math.sin(a) * (100 - i)) / 5) / 4;
        const s = ((162 + 4 * (1 - i)) / 50) * 0.4; // Reduced scale by 60%
        if (e.use) {
      ...
      if (radRef.current < radmRef.current) radRef.current++;
      frmRef.current += 0.003;
      
      if (radRef.current > 60) {
        pointerRef.current.x += (widthRef.current / 2 - pointerRef.current.x) * 0.05;
        pointerRef.current.y += (heightRef.current / 2 - pointerRef.current.y) * 0.05;
      }
    };
